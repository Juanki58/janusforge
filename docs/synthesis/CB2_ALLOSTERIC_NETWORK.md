...
Este documento sintetiza la **red de comunicación alostérica (ACN)** y los nodos microswitch relevantes para CB₂ en el contexto Janusforge. El mapa mecanístico detallado permanece en:
...
**Referencia verificada:** Morales-Pastor, A. *et al.* «Multiple intramolecular triggers converge to preferential G protein coupling in the CB2R.» *Nature Communications* **16**, 5265 (2025). DOI [10.1038/s41467-025-60003-0](https://doi.org/10.1038/s41467-025-60003-0).
...
- Simulaciones MD iniciadas desde estado inactivo **5ZTY**; análisis de contactos en ventanas μs agregadas por mutante.

### Implicación para Janusforge (Nivel 2–3)

La eficacia funcional y el sesgo de vía **no** son función escalar de distancia ortostérica estática. La ACN integra perturbaciones distribuidas — coherente con veredictos **INDETERMINATE** de Phase H y micronetwork en este repo.

---

## 3. Trp258^6.48 — toggle switch y continuo funcional

**Referencia verificada:** Ganzoni, R. L. Z. *et al.* «Single-position ligand modifications tune CB₂R activity by targeting the toggle switch.» *Chemical Science* (2026). DOI [10.1039/D6SC00062B](https://doi.org/10.1039/D6SC00062B).
...
La plasticidad de Trp258 entre estados receptor (6PT0 vs 6KPF) explica por qué el stack estático no unifica el par enantiomérico — ver [`HU308_HU433_PARADOX.md`](HU308_HU433_PARADOX.md).
...
- Nodo clásico de activación class-A; proxy Contract v1.0 (contacto ≤ 3.5 Å).
- Calibración multistate: distancias Ser285 2.7–3.5 Å para HU-308/HU-433/O-1966 según estado (`cb2_multistate_calibration_synthesis.md` §5.1).
- **No** demostrado como requisito universal — Morales-Pastor 2025 muestra múltiples rutas ACN hacia Gαi2 preferencial.

### 4.2 Phe183 (ECL2)

- Desplazamiento ECL2 medido en Phase F (p. ej. 6KPF Δ ~27 Å vs 6PT0).
- Nodo compartido entre rutas vestibulares (Ec21a) y contacto ortostérico en calibración.
- Mutaciones F183A ablacionan PAM Ec21a en literatura (Wang *et al.* 2026, **9U7L**).
...
| **DRY** | TM3 | Bloqueo ionico / activación; nodo de red |
| **NPxxY** | TM7 | Acoplamiento G-proteína; contactos alterados por mutantes |
| **CWxP** | TM6 | Microswitch TM6; acoplado a Trp258 |

Estos motivos conectan la capa macro (Phase G: separación activo/inactivo) con la capa micro (Phase H/micronetwork: INDETERMINATE).
...
| **ORG27569** | NAM CB1 | 6KQI (co-estructuras) | Contacto F237^4.46; switch F155–F237 | Solo CB1 |
| **ZCZ011** | PAM CB1 | 7WV9 | Rearrangement TM2 hacia activo | Solo CB1; **sin actividad alostérica CB2 reportada** |

**Nota de gobernanza:** ORG27569 y ZCZ011 son precedentes **CB1** que refuerzan la pregunta motriz del programa (evitar equivalencia funcional CB1 al modificar CB2) — **no** son plantillas de diseño validadas para CB2 en Janusforge. Ver [`cb2_allosteric_switch_map.md`](../cb2_allosteric_switch_map.md) §Ruta B.

**8X3L** (Shen *et al.* 2024, DOI [10.1073/pnas.2401091121](https://doi.org/10.1073/pnas.2401091121)): complejo CB2–G con implicaciones de selectividad entropy-driven — entra en atlas estructural, no en pipeline activo.
...
| Capa | Estatus | Fuente |
|------|---------|--------|
| Macro TM3–TM6 | 🟢 **DEMOSTRADO** (Phase G GENERALIZES) | `fase_g_generalization_report.md` |
| Micro-red Trp258/Ser285/Phe183 | 🟡 **LÍMITE ESTÁTICO** (INDETERMINATE) | `micronetwork_modes_report.md`, `fase_h_ordinal_functional_report.md` |
...
3. **Cinética k_off** — mecanismo propuesto para Ec21a (literatura); no medido en repo.
4. **Membrana / colesterol** — CLR404 en 6PT0; modulación lipídica documentada en literatura (Ruta C del switch map).
5. **MD μs** — Morales-Pastor 2025 y Ganzoni 2026 usan MD externa; **ninguna MD μs ejecutada post-congelación en Janusforge**.

**Mantra:** incorporar dinámica explícita **antes** de reclamar predictor de eficacia Gi desde geometría estática.

---

## 7. Integración con calibración multistate (proyecto)

La calibración cerrada (`docs/cb2_multistate_calibration_synthesis.md`) demostró **Categoría 1** para HU-433/O-1966: compatibilidad ortostérica conservada en 6PT0/5ZTY/6KPF.

**Esto NO invalida el mapeo alostérico.** Significa que la actividad CB₂ documentada **no requiere** invocar sitios extraortostéricos para explicar acomodación en el panel calibrado. Las rutas A/B/C del switch map permanecen como espacio hipotético futuro.
